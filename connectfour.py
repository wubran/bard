...
class ConnectGame:
    ...
    def boardscorer(self, turn=-1, ftops=None): #make it check the other player's best move, and both player's board's scores after this move
        ...
        for each in range(self.columns):
            ...
            if y == self.rows:
                ...
                boardscores.append(returned[0] + returned[1] + returned[2]*1.5 + returned[3]*1.5)
        return boardscores
    ...
    def futureai(self, ffuture=False, fftops=None):
        if ffuture:
            ftops = fftops
        else:
            ftops = self.tops
        futures = []
        now = self.simpleai(True, ftops)
        if self.turn == 0:
            f = 1
        else:
            f = -1
        for column in range(self.columns):
            x = column
            ftops[x] += 1
            future = self.simpleai(True, ftops)
            futures.append(f*(now[0]-future[0])-f*(now[1]-future[1]))
            ftops[x] -= 1
        if ffuture:
            futurescore = 0
            for item in futures:
    ...
    def doublefutureai(self, fffuture=False):
        ftops = self.tops
        futures = []
        now = self.futureai(True, ftops)
        for column in range(self.columns):
            x = column
            ftops[x] += 1
            futurescore = self.futureai(True, ftops)
            futures.append(futurescore-now)
            ftops[x] -= 1

        lazyscore = self.simpleai(False, ftops, True)
        for index in range(len(futures)):
            futures[index] = int(futures[index]*10) + int(lazyscore[index]*10)*2

        if fffuture == False:
            bestscore = 0
            bestmoves = []
            for index in range(len(futures)):
                if futures[index] > bestscore:
                    bestmoves = [index]
                    bestscore = futures[index]
                elif futures[index] == bestscore:
            ...
            if len(bestmoves) == 1:
                ...
                bestmove = bestmoves[random.randint(0, len(bestmoves) - 1)]
            print(futures)
            print(f"best move is {bestmove + 1}, score of {bestscore}")
            self.x = bestmove
            self.y = self.tops[self.x]
            self.tops[self.x] += 1
            self.array[self.y][self.x] = self.turn + 1
